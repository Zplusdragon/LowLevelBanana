...
def calculate_psnr_kornia(
        img, # lq
        img2, # gt
        max_val=1.0,
        **args
):
    return kornia.metrics.psnr(
        img, img2, max_val=max_val, **args
    ).item()


@METRIC_REGISTRY.register()
def calculate_ssim_kornia(
        img, # lq
        img2, # gt
        window_size: int = 11,
        max_val: float = 1.0,
    ...
    return kornia.metrics.ssim(
        img, img2, window_size=window_size, max_val=max_val, **args
    ).mean().item()
